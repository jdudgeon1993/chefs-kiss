...
<head>
    ...
    <title>SmartPantry - Freezer Master v5.5</title>
...
<body class="bg-gray-100 p-4 md:p-8 font-sans text-gray-900">
    ...
    <div x-data="pantryApp()" x-init="init()" x-cloak class="max-w-5xl mx-auto space-y-6">
        ...
        <nav class="flex border-b border-gray-200 space-x-2 md:space-x-6 overflow-x-auto bg-white p-2 rounded-t-2xl shadow-sm sticky top-0 z-20">
            <template x-for="tab in ['Pantry', 'Recipes', 'Meal Plan', 'Shopping', 'Settings']">
                <button @click="activeTab = tab" 
                        :class="activeTab === tab ? 'tab-active' : 'text-gray-400'" 
                        class="pb-2 px-4 text-xs md:text-sm uppercase tracking-widest transition-all whitespace-nowrap" 
                        x-text="tab"></button>
            </template>
        </nav>
            ...
            <div class="bg-white p-6 rounded-2xl shadow-md border border-gray-200 space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" x-model="forms.pantry.name" placeholder="e.g. Ground Beef" class="w-full border p-2 rounded-xl outline-none focus:ring-2 ring-indigo-100">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <div class="flex">
                            ...
                            <input type="text" x-model="forms.pantry.unit" placeholder="lb/oz" class="w-1/2 border p-2 rounded-r-xl border-l-0">
                        </div>
                    </div>
                    <div>
                    ...
                    <div>
                        <label class="text-[10px] font-black text-gray-400 uppercase">Store In</label>
                    ...
                    <button @click="savePantryItem()" class="md:col-span-1 bg-indigo-600 text-white p-2 rounded-xl font-black hover:bg-indigo-700 transition self-end h-[42px] uppercase text-xs">Add Batch</button>
                </div>
            </div>
                                ...
                                <div class="flex flex-wrap md:flex-nowrap justify-between items-center bg-white p-3 rounded-xl border border-gray-100 shadow-sm gap-4">
                                    ...
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <div class="flex items-center bg-gray-50 rounded-xl p-1 border">
                                            ...
                                            <input type="number" step="0.1" x-model.number="batch.qty" @change="save()" class="w-12 bg-transparent text-center font-black text-sm outline-none">
        ...
        <section x-show="activeTab === 'Recipes'" class="space-y-4">
            ...
            <div class="bg-white p-6 rounded-2xl shadow-md border border-gray-200 space-y-4">
                ...
                <div class="flex gap-2">
                    <input type="text" x-model="forms.recipe.ingName" placeholder="Ingredient Name" class="border p-2 flex-1 rounded-xl text-sm">
                ...
                <div class="flex flex-wrap gap-2">
                    <template x-for="(ing, idx) in forms.recipe.tempIngs">
                        <div class="bg-indigo-50 text-indigo-700 px-3 py-1 rounded-full text-[10px] font-black uppercase flex items-center">
                            ...
                            <button @click="forms.recipe.tempIngs.splice(idx, 1)" class="ml-2 font-bold">&times;</button>
                        </div>
                    </template>
                </div>
                ...
                <template x-for="recipe in recipes" :key="recipe.id">
                    <div class="bg-white p-5 rounded-2xl border-white hover:border-indigo-100 border-2 transition-all shadow-sm">
                        <div class="flex justify-between">
                            ...
                            <button @click="deleteItem('recipes', recipe.id)" class="text-[10px] font-black text-gray-300 hover:text-red-500 uppercase">Delete</button>
                        </div>
        ...
        <section x-show="activeTab === 'Meal Plan'" class="space-y-4">
            <h2 class="text-2xl font-black tracking-tight uppercase">Meal Plan</h2>
                            ...
                            <template x-for="item in items" :key="item.name">
                                <div class="p-6 flex flex-col lg:flex-row justify-between gap-6">
                                    <div class="flex-1">
                                        ...
                                        <span class="text-red-500 text-[10px] font-black uppercase" x-text="'Need: ' + item.shortage + ' ' + item.unit"></span>
                                    </div>
                                    <div class="bg-indigo-50/50 p-4 rounded-2xl grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 items-end border border-indigo-50">
                                        <div>
                                            ...
                                            <input type="number" x-model.number="item.shortage" class="w-full border p-2 rounded-xl text-sm font-black outline-none bg-white">
                                        </div>
                                        <div>
                                            <label class="text-[9px] font-black text-indigo-400 uppercase">Location</label>
                                            <select x-model="item.location" class="w-full border p-2 rounded-xl text-sm font-black outline-none bg-white">
                                                <template x-for="loc in locations">
                                                    <option :value="loc" x-text="loc"></option>
                                                </template>
                                            </select>
                                        ...
                                        <div>
                                            ...
                                            <input type="date" x-model="item.boughtExpiry" class="w-full border p-2 rounded-xl text-sm font-black outline-none bg-white">
                                        </div>
                                        <button @click="checkoutItem(item)" class="bg-indigo-600 text-white px-6 py-3 rounded-xl font-black text-[10px] uppercase shadow-md transition active:scale-95">Check-In</button>
                                    </div>
                                </div>
                            </template>
                        </div>
                ...
                <div class="flex flex-wrap gap-2">
                    <template x-for="loc in locations">
                        <div class="bg-gray-50 px-3 py-1 rounded-lg flex items-center text-[10px] font-black uppercase border border-gray-200">
                            ...
                            <button @click="removeList('locations', loc)" class="ml-2 text-red-500 font-bold">&times;</button>
                        </div>
                    </template>
                </div>
            </div>
        ...
        <div x-show="transferModal.show" class="fixed inset-0 z-50 flex items-center justify-center p-4 modal-bg" x-transition x-cloak>
            <div class="bg-white rounded-3xl shadow-2xl max-w-sm w-full p-8 border border-gray-100" @click.away="transferModal.show = false">
                <h3 class="text-xl font-black text-gray-800 mb-2 uppercase">Move Stock</h3>
                <p class="text-[10px] font-bold text-gray-400 uppercase mb-6" x-text="'Currently in: ' + (transferModal.batch ? transferModal.batch.location : '')"></p>
                
                <div class="space-y-4">
                    <div>
                        <label class="text-[10px] font-black text-indigo-500 uppercase">Amount to Transfer</label>
                        ...
                        <p class="text-[9px] text-gray-400 mt-1" x-text="'Max: ' + (transferModal.batch ? transferModal.batch.qty : 0)"></p>
                    </div>
                    <div>
                        <label class="text-[10px] font-black text-indigo-500 uppercase">New Location</label>
                        <select x-model="transferModal.newLoc" class="w-full border-2 border-indigo-50 p-3 rounded-xl font-black text-sm outline-none">
                            <template x-for="loc in locations">
                                <option :value="loc" x-text="loc"></option>
                            </template>
                        </select>
                    ...
                    <div class="flex gap-2 pt-4">
                        ...
                        <button @click="executeTransfer()" cla            <template x-for="tab in ['Pantry', 'Recipes', 'Meal Plan', 'Shopping', 'Settings']">
